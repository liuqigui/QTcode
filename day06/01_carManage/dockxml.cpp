...
//创建XML文件
void DockXML::createXML(QString filePath)
{
...
void DockXML::appendXML(QString filePath, QStringList list)
{
    QFile file(filePath);
    ...
    if(true == isOk)
    {
        ...
        if(isOk)    //如果关联成功
        {
            ...
            QString dateStr = date.toString("yyyy-MM-dd");  //2023-09-26
...
void DockXML::writeXML(QDomDocument &doc, QDomElement &root, QStringList &list)
{
    //获取当前时间
...
void DockXML::readXML(QString filePath, QStringList &fList, QStringList &bList, QStringList &pList, QStringList &nList, QStringList &tList)
{
    QFile file(filePath);
    bool isOk = file.open(QIODevice::ReadOnly);
    if(true == isOk) //打开成功
